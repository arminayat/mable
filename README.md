...
Set `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` as secret environment variables in Coolify. The Compose file asks Coolify to generate and retain the PostgreSQL password, Better Auth secret, and 32-byte Base64 credential-encryption key. Preserve the PostgreSQL volume and all three generated values across redeployments or a new installation. The web process runs database migrations before it starts; the worker waits for the web health check. Configure the Google web client's redirect URI as `https://mable.arminayat.dev/api/auth/callback/google`, enable Gmail API, and add test users while the OAuth app is in testing mode. Set up database backups before storing real accounts.
