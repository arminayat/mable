...
- Better Auth has `encryptOAuthTokens: true`. AI provider keys use application AES-256-GCM with a random 12-byte IV and authentication tag, keyed by a 32-byte decoded environment key. API responses expose only `hasKey` and the non-secret `keyProvider`. Key/provider replacement is atomic; there is one saved credential per user.
...
- Run SSE authorizes the owned run at connection and rechecks ownership during polling; 50-second reconnects recheck the session. Preview requests additionally verify message membership in the owned run before using that account’s Gmail token. Only subject/from/snippet is returned with no-store; client preview requests and stream subscriptions are cancelled on unmount. Scores/actions are persisted, email content is not.
- Normal model context leaves the application for TypeSafe, either directly or through Vercel AI Gateway as selected, with sender/recipient/subject/date, account email, and up to 20,000 body characters. PostgreSQL has no email-content columns. MIME attachment exclusion is incomplete; see K1.
