...
`pnpm typecheck`, `pnpm lint`, `pnpm test`, `pnpm build` are repository checks. Unit coverage includes encryption, threshold/model validation, provider routing, Gmail helpers and icon morph geometry. CI also migrates disposable PostgreSQL and runs `pnpm test:integration` (worker and credential API cases: discovery, cutoff, dedupe, expired history, run serialization, saved decisions, batches, model failure/retry, cancellation, pause, user separation/deletion, provider routing, encrypted replacement, legacy defaults, secret-free views, authenticated run-history pagination, streamed progress/replay, full ordered result snapshots, preview ownership/content boundaries, stable rediscovery positions and disconnect cleanup).
...
Current local verification (2026-09-25): 35 unit and 26 disposable-PostgreSQL integration tests pass, including authenticated run-history paging and user isolation, real SSE stream reads and the last-50 cap, small inboxes, complete 25-result snapshots across worker batches, API acceptance and interrupted discovery with mocked external services. Typecheck, lint and production build pass. The continuous run review needs no migration, environment variable or worker restart; the web process must run the updated source. The `0000` → `0001` upgrade preserved an existing ciphertext and backfilled its provider to TypeSafe; `0002` also passed migration/integration checks. Both migrations were applied locally and during the earlier production release.

Production release `df2d273614d974267519c528649c07a9092093bf` passed [GitHub CI 36169383650](https://github.com/arminayat/mable/actions/runs/36169383650), including migration and integration checks on disposable PostgreSQL. Coolify application `mkqjvrpgpp12xc3ho1rk9mmt` deployed that revision successfully in deployment `qjcojb5xhh0u6bictr1mxae0` on 2026-09-25 at 17:50 UTC. Web logs confirmed migrations applied successfully before Next started; DB and web became healthy and the worker started. The public homepage and privacy page returned HTTP 200, the deployed privacy page includes Gateway wording, and the new run-stream route returned HTTP 401 without a session. No new environment variables were needed. The dashboard is `https://coolify.arminayat.dev`; deployment was triggered manually after CI passed on pushed main. A subsequent documentation-only commit records this evidence without requiring another application rollout.

Authenticated browser interactions, visual animation acceptance, and live Gmail/Jev/Gateway evaluation were not tested during this release. Deployment and startup evidence do not establish those acceptance gates. No local worker was running at the local verification; start `pnpm worker` separately to process local runs. Production runs use the deployed worker.

## Planned/aspirational

Backup/restore automation, automated deployment verification, public operator details, retention policies and live provider acceptance are not established here. Documentation changes require no migration, environment change, or deployment step; future runtime changes should report their own operational requirements.
