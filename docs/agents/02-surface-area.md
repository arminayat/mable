...
| `/` signed in | `src/components/app.tsx` | Fetches `/api/mable`, shows ordered questions and latest run; creates, edits, deletes, toggles, reorders rules through POST commands. Header History opens earlier runs. Polls every 3 seconds when queued/running, otherwise 30 seconds. |
...
| Run dialog/status | `src/components/run-dialog.tsx`, `run-progress-view.tsx`, `use-run-stream.ts` | Selects new/latest (last 50 inbox emails)/recent/all, creates a persisted run, expands into live processed/total progress and two-column email/evaluation review. Uses SSE plus on-demand Gmail previews and 20-row result pages. Closing does not stop the worker. Stop/discard and retry are explicit commands. The latest run can be reopened. |
...
| `GET /api/mable/runs/[id]/events` | `src/app/api/mable/runs/[id]/events/route.ts` | Authenticated, owned-run SSE snapshots; optional zero-based `page`; about 1s polling, 50s connections, no-store. Terminal snapshots close the stream. |
...
Run now opens setup for a new run, or reopens an active/unfinished run from a Play icon beside the header user menu. It is hidden when there are no saved questions; otherwise it is disabled without a saved key, Gmail connection, or enabled question. Existing/unfinished-run conflicts are enforced server-side. The main view returns/displays only the newest run; the separate History button lists older runs for review. No separate settings/rules/run pages, webhook, public versioned API, or health endpoint is present.
