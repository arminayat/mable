...
5. Starting a run expands the dialog into a live two-column review: Gmail subject/sender/snippet on the left, Jev probabilities and selected/applied actions on the right. An authenticated SSE connection sends DB-backed progress snapshots about once per second, with processed/total counts in the header. All results remain in one scroll list during and after processing; the latest run can be reopened. Users can stop active runs or retry/discard failed and paused runs.
...
- There are no reply/send/delete-email actions, dry-run preview, undo, IMAP/Outlook provider, billing, or team/org model. Header History lists previous runs, and each can open its per-email review UI.
