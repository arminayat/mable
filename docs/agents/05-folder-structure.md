...
| `src/components/` | Account UI, question editor, settings and sign-in; shared UI view types in `types.ts`. Private row/icon helpers stay in `app.tsx`. |
| `src/lib/` | Schema/DB, auth, credential crypto, run lifecycle, worker execution, Gmail and evaluation services. Shared processing logic belongs here, not in the process loop or browser. |
...
| `tests/integration/` | Real disposable-DB worker tests selected separately. |
...
| Root configs | Manifest/lock, Next/TypeScript/ESLint/PostCSS/Drizzle configs, Dockerfile, both Compose files, `.env.example`. |
