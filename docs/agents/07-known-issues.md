...
### K10 — billing emails skipped by invoice rule (production diagnosis; behavior unchanged)

- **Observed 2026-09-25:** the latest production review showed 50 processed, 0 changed, 50 skipped and 0 failed. Its single snapshotted question was `Is it an invoice?`, with a 90% threshold. Billing examples scored 67% (receipt), 60% (upcoming invoice notice), 31% (booking receipt) and 21% (payment confirmation); all were below threshold. Worker runtime logs contained startup output and no displayed errors. Current Settings showed Vercel AI Gateway.
