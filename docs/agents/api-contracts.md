...
`GET /api/mable/runs/[id]/events` requires a session and an owned run (401/404 otherwise). Optional `page` is a nonnegative zero-based integer (400 for invalid values); large pages clamp to the last page. Omission follows the active email or latest processed page. `event: progress` carries `{ id, status, discovered, error, total, processed, changed, skipped, failed, threshold, rules, page, pageCount, pageSize, items }`; items contain ID, Gmail ID, position, state, phase, nullable probabilities and nullable actions. Page size is 20. Counts come from message rows; no credentials or email content is streamed. Snapshotted questions/threshold explain the original decision even after rule edits.
