...
Model context is `state: { email: { from, to, subject, date, owner, body } }`: from/to/subject/date headers, account email (`owner`), and normalized body capped at 20,000 characters. Rule questions are passed verbatim as boolean instructions; no additional invoice definition or classification prompt is added. Gmail labels, attachment names and PDF contents are not sent. Review snippets come from a separate metadata fetch and do not establish the exact evaluated context; historical request bodies are not persisted. MIME recursion accepts plain and HTML text (HTML tags removed by regex); it does not implement robust attachment exclusion or MIME alternative selection. See K1.
